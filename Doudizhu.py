...
class Doudizhu:
    ...
    def toBeLandholder(self):
        ...
        self.dizhu=n # Mark for landholder
        if n==0:
            self.str1+=self.str4
        if n==1:
            self.str2+=self.str4
    ...
    def airkikGra(self,card):
        ...
        while (index < size - 2):
            if (self.c_grades[card[index]] == self.c_grades[card[index + 1]] and \
                ...
                fliterList.append(card[index + 2])
            index += 1
        return size,self.airGra(fliterList)
...
user.mapping()
dizhu = user.dizhu
print 'dizhu: User#{0:1d}'.format(dizhu)
...
out,type = oneturns(dizhu,None,None,None,None)
# print(out)
